...
- **实时频谱（本轮重构为山峰风格，分支 `codex/spectrum-peaks`）**：`EQSpectrumRing`（16384 样本环形缓冲 + NSLock，音频线程写入下混单声道、prepare 时写入采样率）→ `EQSpectrumAnalyzer`（主线程 30fps 定时器，4096 点 Hann 加窗 + vDSP 实数 FFT，20Hz~20kHz 对数频轴 96 点峰值聚合，dB 归一化窗口 -72~-6dB + 高频倾斜补偿 +10dB 对数 tilt + 幂次 ^1.35 对比拉伸，快攻慢放 0.75，暂停时谱线衰减归零）→ `EQSpectrumView` Canvas 绘制：弱张力（1/9）Catmull-Rom 峰形曲线 + 渐变填充 + 矮化慢衰减「远山影」第二层（`trailing`）+ 频率刻度；移除旧版人工水波基线、相位回声波与 phase 属性，轮廓随真实频谱高低错落。纯本地 Accelerate/vDSP 计算。
