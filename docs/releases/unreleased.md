...
- 歌曲列表改为固定分栏布局：标题列固定 500pt、专辑列固定 260pt 靠左排列（过长专辑名省略号截断），时长列后右侧让出空余区域；仅在专辑/时长之间显示一条 13pt 高静态灰色短竖线（只在列头高度内），标题/专辑之间不再有分隔线，列宽不可拖拽调整。列头「标题」「专辑」文字与下方歌曲行严格左对齐；列头选择按钮与每行爱心/更多按钮固定对齐（取消此前多选时的 -16pt 左移）。专辑/艺术家详情页与播放页队列列表（无列头）保持原弹性布局不变。
...
- 多选状态从 `MainView` 下沉到 `TrackListView` 内部：`@State private var isSelectionMode / selectedIDs` 由列表自管，`TrackRow` 新增 `isSelectionMode / isSelected / onToggleSelection`（多选时行首显示勾选圆钮、点击行切换选中、选中行主题色底）；列头首列占位在多选时由 44pt 变为 84pt（28 勾选框 + 12 间距 + 44 封面），使列头文字与歌曲行内容同向同幅右移 40pt 并保持列对齐，不再向左收缩；列头「时长」右侧新增固定 72pt 宽 `ZStack` 编辑区（与行尾爱心+更多区同宽）：入口 `checkmark.circle` 图标钮 `offset(x: 10)` 与下方爱心中心对齐，编辑态变实心高亮且再点退出；全选/添加到歌单/删除/取消四个 15pt 宽图标钮作为浮层 `offset(x: 28)` 从右侧以 `.move(edge: .trailing) + opacity` 滑出，不参与 HStack 布局、不挤占列宽。
...
- 固定分栏：`TrackListView` 列头为 `HStack(spacing: 0)` 显式固定布局（封面位 44/84 + 12 间隙 + 标题 500 + 12 + 专辑 260 + 12 分隔热区 + 时长 48 + Spacer + 操作区 72）；专辑/时长间隙为 `columnGapWithDivider`（12pt 宽内 overlay 一条 13pt 高、`hpTextPrimary` 0.16 透明度的静态灰色短竖线，无任何手势/悬停交互）。拖拽调宽方案（`@AppStorage` 列宽、`DragGesture`、`NSCursor.resizeLeftRight`、全高 overlay 高亮线）已全部移除；`ColumnFrame` ViewModifier 保留，`TrackRow` 可选 `titleWidth / albumWidth`（nil 时退化为 maxWidth infinity 弹性布局，供无列头的队列列表复用）；列头操作 `ZStack` 与 `TrackRow` 右侧 72pt 操作区不再做 -16pt 偏移，选择图标与爱心始终同位对齐。
