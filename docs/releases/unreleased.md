...
- **调整（音量控件搬家）**：底部播放条右侧原音量控件（窄屏：喇叭+数字+滚轮；宽屏：喇叭+76pt 滑杆）删除，替换为原播放页同款浮出式控件——新增 `PlayerVolumeControl`（封装展开/收起状态与 NSEvent 点击外部收起监视器），点击喇叭后从**上方弹出竖向滑块**（`VerticalVolumeSlider`：3.5pt 竖轨道 + 主题色填充 + 12pt 圆形拇指，点击轨道跳转、按住上下拖动调节，上方为最大音量），再点喇叭收起，收起时悬停滚轮调节、喇叭下方显示音量数字；收起判定热区随滑块改为向上扩展。加 `zIndex(1)` 保证悬浮滑块绘制在相邻控件之上。播放页（`NowPlayingView`）控制行删除音量按钮及相关状态，监视器挂载/卸载随控件迁入 `PlayerVolumeControl`；`VolumeFrameReporter`/`VolumeScrollCatcher` 两个辅助视图仍留在 NowPlayingView.swift（模块内访问）。
