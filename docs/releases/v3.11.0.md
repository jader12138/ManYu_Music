...
- 对应 `VERSION`：`3.11.0`
- 上一稳定版本：`v3.10.0`
- 发布分支：`codex/v3.11.0-release`

## 摘要

v3.11.0 是一轮以性能、稳定性与内存为主的版本：大幅顺滑了首页与播放页之间的转场（封面按入口区分 hero 动效、背景模糊与光斑预烘焙、完整播放页启动预热），把大量切歌后的驻留内存从约 290MB 压到约 170MB 并新增系统内存压力兜底；同时带来启动封面预加载开关、「回到顶部」悬浮按钮、播放页音量滑块、歌词进度微调、收藏爱心等新功能，并修复了部分 FLAC 进度条拖到末尾后音乐仍多播十几秒、歌词与歌声对不齐的问题。

## 用户可见更新

### 新增

- 「回到顶部」悬浮按钮：在歌曲页、首页等内容页向下滚得较远（距顶超过约 300pt）后，只要向上滚动一下，内容区右下角会出现悬浮胶囊按钮（浮在所有页面之上、不嵌进任何页面），点击后平滑滚回列表顶部；向下滚动或距顶部不足 80pt 时自动隐藏，侧栏与播放队列的滚动不会触发。
...
- `LibraryStore` 新增会话级 `sessionRecommendationTrackID`："每次打开"模式的首页推荐在整个运行期间稳定，重启应用重新挑选。
...
- 新增 `Tests/HarmonyPlayerTests`：资料库持久化、浏览快照（含性能基准）、封面管线、播放时钟隔离等测试，当前共 27 个测试。

## 兼容性与迁移

- 最低系统版本仍为 macOS 14。
- 无数据格式或用户配置迁移；`library.json` 持久化格式保持不变，旧文件可正常加载。
- 封面缓存预算调整为 48MB / 320 条，旧缓存自动按新预算淘汰，无需用户处理。

## 验证

- 发布最终验证：在 v3.11.0 发布合并结果上 `swift test` 27/27 通过；`scripts/build-app.sh` 以 `VERSION = 3.11.0` 重新 Release 构建，应用包 `Info.plist` 的 `CFBundleShortVersionString = 3.11.0`、`CFBundleVersion` 为当时提交计数；`codesign --verify --deep --strict` 签名校验通过；在合并提交上创建 annotated 标签 `v3.11.0`（标签信息「漫域音乐 v3.11.0」）；`git push origin main` 与 `git push origin v3.11.0` 后以 `git ls-remote` 核验远端分支与标签均已就位；GitHub Release「漫域音乐 v3.11.0」已创建。
...
- 播放页转场丝滑化分支（codex/nowplaying-warmup，叠加 codex/nowplaying-transition 与 codex/hero-transition 已合并内容）合并前：以临时探针实测（2ms 主线程心跳看门狗 + 事件打点）定位出首次进入播放页 191ms 冷启动停顿与歌词挂载 26-39ms 尾部顿挫；实施完整播放页启动预热、光斑预烘焙与歌词延迟挂载后复测无主线程停顿；移除探针后测试通过，Release 构建签名后实际运行验证：重复进出播放页无可感顿挫，首次点击仅剩切歌内容中途换入的极轻微感（收益递减，未再深挖）。
- `swift build` 通过（macOS 14, arm64）。
- 收藏爱心与歌词/进度条重构分支（codex/favorite-lyrics-smoothness）合并前：Release 构建签名后实际运行验证——歌词滚动与点击跳转连续缓动、进度条无回跳抖动、收藏状态两处同步。
- 歌词偏移与 credits 过滤分支（codex/lyric-offset）合并前：测试通过（含新增解析器测试：credits 过滤、offset 标签、多时间戳、小数精度、纯文本回退）；Release 构建签名后实际运行验证进度微调实时生效、按歌记忆、面板不跳动。
- 播放页视觉与音量分支（codex/nowplaying-cover-gradient）合并前：Release 构建签名后实际运行验证标题渐变随封面切换、背景缓慢流动、音量滑块展开/拖动/点击外部收起全链路（热区外点击收起且功能穿透、滑块内拖动不误收）。
